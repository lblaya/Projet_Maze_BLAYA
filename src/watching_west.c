...
static int verif_start_west(int idx, int y, char *dynamic_map)
{
...
static struct coords *verif_watching_west(int idx,
                                          int y,
...
struct coords *watching_west(int width, struct coords *map_info)
{
    ...
    idx = map_info->idx;
    y = width + 1;
    if (verif_start_west(idx, y, map_info->dynamic_map) == 1) {
        map_info->dynamic_map[idx] = ' ';
        idx -= 1;
    }
    ...
    map_info->idx = idx;
    map_info = verif_watching_west(idx, y, map_info);
    return map_info;
}
